...
const Signup = () => {
    const { mutate: signUpWithEmail, isLoading } = trpc.registerUser.useMutation()
    const {
        control,
        handleSubmit,
        formState: { errors },
    ...
    return (
        <SafeAreaView>
            <ScrollView className="p-5 bg-[#FFFF] h-screen">
                <Image source={require("../../assets/signup.png")} className="h-56 w-56 mx-auto " />
                ...
                <Card>
                    ...
                    <Button className="mt-3" onPress={handleSubmit(handleSignUp)}>
                        {isLoading ? (
                            ...
                            <Text className="text-center text-white font-bold py-2">Sign up</Text>
                        )}
                    </Button>
                </Card>
